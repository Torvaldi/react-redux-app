...
// if (appConfig.config.prod) {
//   let options = {
//     key: fs.readFileSync(
//       "/etc/letsencrypt/live/socket.guesstheanimeopening.com/privkey.pem"
//     ),
//     cert: fs.readFileSync(
//       "/etc/letsencrypt/live/socket.guesstheanimeopening.com/cert.pem"
//     ),
//     ca: fs.readFileSync(
//       "/etc/letsencrypt/live/socket.guesstheanimeopening.com/chain.pem"
//     ),
//   };
...
io.on("connection", (socket) => {
  ...
  socket.on(event.USER_JOIN_GAME, (data) => {
    ...
    socket
      .to(ioHelper.getRoom(game.id))
      .emit(event.USER_JOIN_GAME, player.serialize());
  });
  ...
  socket.on(event.USER_POST_CHAT, (data) => {
    ...
    socket
      .to(ioHelper.getRoom(gameId))
      .emit(event.USER_POST_CHAT, { player, message });
  });
  ...
  socket.on(event.CHANGE_STATUS_0_TO_1, (data) => {
    ...
    setTimeout(() => {
      ...
      if (currentGame.getGameStatus() !== statusHelper.gameStatus.loading)
        return;
  ...
  socket.on(event.CHANGE_STATUS_1_TO_2, (data) => {
    ...
    // prevent from sending the event if the game status is not the expected one
    if (currentGame.getGameStatus() !== statusHelper.gameStatus.musicPLaying)
      return;
    ...
    setTimeout(() => {
      ...
      let isFastPassAfterTimeout = currentGame
        .getLastTurn()
        .getFastPassAnswer();
      let turnNumberAfterTimeout = currentGame.getTurnNumber();

      if (
        isFastPassAfterTimeout !== isFastPassBeforeTimeout ||
        turnNumberAfterTimeout !== turnNumberBeforeTimeout
      )
        return;
      ...
      io.in(ioHelper.getRoom(gameId)).emit(event.CHANGE_STATUS_1_TO_2, {
        turnResult,
        players,
      });
    }, timeout);
  ...
  socket.on(event.CHANGE_STATUS_2_TO_0, (data) => {
    ...
    if (winners.length > 0) {
      ...
      io.in(ioHelper.getRoom(gameId)).emit(event.GAME_FINISH, {
        winners,
        animes,
      });
    ...
    setTimeout(() => {
      // prevent from sending the event if the game status is not the expected one (it might have change since the previous check)
      if (currentGame.getGameStatus() !== statusHelper.gameStatus.result)
        return;
      ...
      if (
        isFastPassBeforeTimeout !== isFastPassAfterTimeout ||
        turnNumberAfterTimeout !== turnNumberBeforeTimeout
      )
        return;
  ...
  socket.on(event.USER_LEAVE_GAME, (data) => {
    ...
    if (currentGame.getCreatorUsername() !== player.username) {
      ...
      socket
        .to(ioHelper.getRoom(gameId))
        .emit(event.USER_LEAVE_GAME, { player });

      // the user leaving the game is the creator
    } else {
      api.updateGameStatus(token, gameId, 4).then((statusCode) => {
...
http.listen(appConfig.config.port, () => {
  console.log("listening on *:" + appConfig.config.port);
});
